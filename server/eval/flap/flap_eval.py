...
[채점 규칙]
- 이벤트 종류: flap 만 정답으로 인정
- 평가 기준:
  1. flap 이벤트 인식 (0/2점)
  2. 시간 범위 반영 (0~3점)
  3. 수치 일치(total_flaps, flap_count 등) (0~3점)
  4. 설명 품질 (0~2점)
...
# 채점 함수
# -------------------------------
def grade_flap(question, ideal, answer, success=True):
...
if __name__ == "__main__":
    gt_file = "test1.jsonl"                # 정답지 JSONL
    ans_file = "flap_model_answers.jsonl"  # LLM 답변 JSONL
    out_file = "flap_graded_results.jsonl" # 결과 저장
    ...
    for gt, ans in zip(gt_data, ans_data):
        ...
        score = grade_flap(q, ideal, model_answer, success=success_flag)
        results.append({
            "input": q,
            "ideal": ideal,
