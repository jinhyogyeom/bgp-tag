...
class FlapRecord:
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> Optional['FlapRecord']:
        try:
            ...
            flap_count = safe_int_convert(data.get('flap_count'))
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate 10-minute interval natural language JSONL reports from TimescaleDB flap analysis results"
    )
    parser.add_argument(
        '--output_file', '-o', default='flap_10min_nl_reports.jsonl',
...
def fetch_flap_records(start_time: str, end_time: str, batch_size: int) -> List[FlapRecord]:
    """
    TimescaleDB에서 flap 분석 결과를 배치 단위로 조회
    """
    records = []
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            ...
            while True:
                ...
                for record in batch:
                    if flap_record := FlapRecord.from_dict(record):
                        records.append(flap_record)
...
def calculate_risk_score(events: int, flap_count: int, duration_hours: float) -> float:
    """
    Calculate normalized risk score based on event density, flap count, and duration
    """
    ...
    normalized_flaps = min(flap_count * 0.3, 30)       # flap 횟수 제한
    normalized_duration = min(duration_hours * 0.3, 24) # 지속 시간 제한
    
    return normalized_density + normalized_flaps + normalized_duration

def process_bucket(records: List[FlapRecord], timezone: str) -> Dict[str, Any]:
    """
    ...
    total_flaps = sum(r.flap_count for r in records)

    # Aggregate per-prefix metrics
    prefix_groups: Dict[str, List[FlapRecord]] = {}
    for rec in records:
    ...
    details = "; ".join(
        f"{d['prefix']} had {d['total_events']} updates with {d['flap_count']} flaps over {d['duration_hours']:.1f} hours" 
        for d in top_agg
    )
    ...
    rpt = (
        ...
        f"{total_prefixes} unique prefixes showed flap activity, totaling {total_events} updates with {total_flaps} flaps. "
