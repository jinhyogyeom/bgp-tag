...
class LoopRecord:
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> Optional['LoopRecord']:
        try:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate 10-minute interval natural language JSONL reports from TimescaleDB loop analysis results"
    )
    parser.add_argument(
        '--output_file', '-o', default='loop_10min_nl_reports.jsonl',
...
def fetch_loop_records(start_time: str, end_time: str, batch_size: int) -> List[LoopRecord]:
    """
    TimescaleDB에서 loop 분석 결과를 배치 단위로 조회
    """
    records = []
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            ...
            while True:
                ...
                for record in batch:
                    if loop_record := LoopRecord.from_dict(record):
...
def calculate_risk_score(events: int, path_length: int, duration_hours: float) -> float:
    """
    Calculate normalized risk score based on event density, path length, and duration
    """
    ...
    normalized_path = min(path_length * 0.3, 30)       # 경로 길이 제한
    normalized_duration = min(duration_hours * 0.3, 24) # 지속 시간 제한
    
    return normalized_density + normalized_path + normalized_duration

def process_bucket(records: List[LoopRecord], timezone: str) -> Dict[str, Any]:
    """
    ...
    total_paths = len({tuple(r.as_path) for r in records})

    # Aggregate per-prefix metrics
    prefix_groups: Dict[str, List[LoopRecord]] = {}
    for rec in records:
    ...
    details = "; ".join(
        f"{d['prefix']} had {d['total_events']} updates with {d['path_count']} unique AS paths over {d['duration_hours']:.1f} hours" 
        for d in top_agg
    )
    ...
    rpt = (
        ...
        f"{total_prefixes} unique prefixes showed AS path loop activity, totaling {total_events} updates with {total_paths} unique AS paths. "
