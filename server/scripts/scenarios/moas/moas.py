...
def parse_arguments():
    parser = argparse.ArgumentParser(description="BGP MOAS Analysis Summarization by Prefix for RAG")
...
def fetch_bgp_updates(start_time: str, end_time: str) -> pd.DataFrame:
    """
    PostgreSQL에서 BGP 업데이트 데이터를 조회하여 MOAS 분석에 필요한 형태로 변환
    """
...
⚠️ MOAS detected! Multiple origin ASNs for the same prefix.
""".strip())
...
def save_to_timescale(conn, summaries):
    ...
    try:
        ...
        execute_values(cursor, """
            INSERT INTO moas_analysis_results 
            (time, prefix, origin_asns, total_events, first_update, 
...
def main():
    ...
    try:
        ...
        # MOAS 분석 수행
        summaries = analyze_moas_anomalies(df)
