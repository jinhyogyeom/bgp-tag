...
class MoasRecord:
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> Optional['MoasRecord']:
        try:
            # 숫자 필드 안전하게 변환
            total_events = int(float(str(data.get('total_events', 0))))
            
            return cls(
                time=parse_datetime(data['time']),
                prefix=data['prefix'],
                origin_asns=data['origin_asns'],
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate 10-minute interval natural language JSONL reports from TimescaleDB MOAS analysis results"
    )
    parser.add_argument(
        '--output_file', '-o', default='moas_10min_nl_reports.jsonl',
...
def fetch_moas_records(start_time: str, end_time: str, batch_size: int) -> List[MoasRecord]:
    """
    TimescaleDB에서 MOAS 분석 결과를 배치 단위로 조회
    """
    records = []
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            ...
            while True:
                ...
                for record in batch:
                    if moas_record := MoasRecord.from_dict(record):
                        records.append(moas_record)
...
def process_bucket(records: List[MoasRecord], timezone: str) -> Dict[str, Any]:
    """
    ...
    total_origin_asns = len(set().union(*[set(r.origin_asns) for r in records]))

    # Aggregate per-prefix metrics
    prefix_groups: Dict[str, List[MoasRecord]] = {}
    for rec in records:
    ...
    for prefix, group in prefix_groups.items():
        ...
        origin_asns = set().union(*[set(r.origin_asns) for r in group])
    ...
    rpt = (
        ...
        f"{total_prefixes} unique prefixes showed MOAS activity, totaling {total_events} updates from {total_origin_asns} unique origin ASNs. "
